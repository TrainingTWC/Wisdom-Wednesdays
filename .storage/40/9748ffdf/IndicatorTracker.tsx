    ...
    return (
      <div className="grid grid-cols-4 gap-2">
        {indicators.map((indicator) => {
          ...
          return (
            <div key={indicator.key} className={`${indicator.bgColor} border ${indicator.borderColor} rounded-lg p-2 text-center`}>
              <Icon className={`w-4 h-4 ${indicator.color} mx-auto mb-1`} />
              <p className="text-xs font-semibold text-gray-700">{indicator.label}</p>
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
      {indicators.map((indicator) => {
        ...
        const Icon = indicator.icon;
        return (
          <Card key={indicator.key} className={`${indicator.bgColor} border-2 ${indicator.borderColor} p-3`}>
            <div className="flex flex-col items-center text-center">
              <Icon className={`w-6 h-6 ${indicator.color} mb-2`} />
              <p className="text-xs font-bold text-gray-700 mb-1">{indicator.label}</p>

      ...
      <Card className="border border-neutral-200/50 bg-white shadow-sm">
        <CardHeader className="text-center pb-4 px-4 md:px-6 pt-4 md:pt-6">
          <div className="flex justify-center mb-4">
            <div className="text-6xl md:text-7xl">{earnedBadge.emoji}</div>
          </div>
          <CardTitle className="text-xl md:text-2xl font-semibold text-neutral-900 mb-2 tracking-tight">Assessment Complete!</CardTitle>
          <p className="text-xs md:text-sm text-neutral-600 font-medium">Your Visual Empathy Performance</p>
        </CardHeader>
        <CardContent className="space-y-4 md:space-y-5 px-4 md:px-6 pb-4 md:pb-6">
          {/* Score Display */}
          <div className={`p-4 md:p-6 rounded-xl ${performanceLevel.bg} border border-neutral-200/50`}>
            <div className="text-center mb-3 md:mb-4">
              ...
              <div className="flex items-center justify-center gap-2 mb-2">
                ...
                <p className={`text-4xl md:text-5xl font-bold ${totalScore >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                  {totalScore > 0 ? '+' : ''}
                  {totalScore}
                </p>
              </div>
              ...
              <p className={`text-lg md:text-xl font-semibold ${performanceLevel.color}`}>{performanceLevel.level}</p>
            </div>
          ...
          {/* Badge Earned */}
          <div className="bg-gradient-to-r from-purple-50 to-pink-50 p-4 md:p-5 rounded-xl border border-purple-200/50">
            <div className="flex items-center gap-3 md:gap-4">
              <div className="text-4xl md:text-5xl flex-shrink-0">{earnedBadge.emoji}</div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-2">
                  <Trophy className="w-4 h-4 md:w-5 md:h-5 text-amber-600 flex-shrink-0" />
                  <p className="font-semibold text-base md:text-lg text-neutral-900 truncate">{earnedBadge.name}</p>
                </div>
                <p className="text-neutral-700 text-xs md:text-sm">{earnedBadge.description}</p>
              </div>
            </div>
          </div>

          {/* Indicator Breakdown */}
          <div className="bg-white p-4 md:p-5 rounded-xl shadow-sm border border-neutral-200/50">
            <h3 className="text-base md:text-lg font-semibold text-neutral-900 mb-3 md:mb-4 text-center">Performance Indicators</h3>
            <IndicatorDisplay scores={indicatorScores} maxPerIndicator={maxPerIndicator} />
          </div>

          {/* Personalized Feedback */}
          <div className="bg-white p-4 md:p-5 rounded-xl shadow-sm border-l-4 border-amber-500">
            <p className="font-semibold text-sm md:text-base text-neutral-900 mb-2 md:mb-3">📊 Performance Analysis</p>
            <p className="text-neutral-700 leading-relaxed text-xs md:text-sm">{getFeedbackMessage()}</p>
          </div>

          {/* Indicator Insights */}
          <div className="bg-gradient-to-r from-blue-50 to-purple-50 p-4 md:p-5 rounded-xl border border-blue-200/50">
            <p className="font-semibold text-neutral-900 mb-3 text-center text-xs md:text-sm">💡 Key Insights</p>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 md:gap-3 text-xs">
              <div className="bg-white p-2.5 md:p-3 rounded-lg border border-neutral-200/50">
          ...
          <div className="flex flex-col sm:flex-row gap-3 pt-4">
            <Button 
              onClick={onRestart} 
              size="lg"
              className="flex-1 bg-gradient-to-r from-amber-500 to-orange-600 hover:from-amber-600 hover:to-orange-700 text-white py-5 md:py-6 text-sm md:text-base font-medium shadow-lg shadow-amber-500/20 rounded-xl transition-all duration-300 min-h-[44px]"
            >
            ...
            <Button
              ...
              variant="outline"
              size="lg"
              className="flex-1 border-2 border-neutral-300 text-neutral-700 hover:bg-neutral-50 py-5 md:py-6 text-sm md:text-base font-medium rounded-xl transition-all duration-300 min-h-[44px]"
            >
              Review Training
            </Button>
          </div>
          ...
          {/* Motivational Quote */}
          <div className="bg-gradient-to-r from-amber-50 to-orange-50 p-3 md:p-4 rounded-xl border-l-4 border-amber-500 text-center">
            <p className="text-sm md:text-base font-semibold text-neutral-900 italic">

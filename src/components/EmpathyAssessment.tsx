...
const scenarios: Scenario[] = [
  ...
  },
  {
    ...
    options: [
      ...
      {
        id: 'd',
        text: "Say: 'Hi! New here? Our most popular drinks are the French Vanilla latte, Dry hazelnut cappuccino, and Orange zest mocha. Want to try one of those?'",
        score: 2,
...
export default function EmpathyAssessment() {
  ...
  return (
    <div className="space-y-5">
      {/* Sticky Progress Header with Indicators */}
      <div className="sticky top-0 z-40 pt-2 pb-2 backdrop-blur-xl bg-gradient-to-b from-white via-white/95 to-white/80">
        <Card className="border border-neutral-200/50 bg-white/90 backdrop-blur-sm shadow-lg">
          <CardContent className="py-4 space-y-3">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <Eye className="w-4 h-4 text-blue-600" />
                <span className="font-semibold text-neutral-900 text-xs md:text-sm">Progress</span>
              </div>
              <div className="flex items-center gap-2">
                <span className="text-xs font-medium text-neutral-600">Score:</span>
                <span className={`text-lg md:text-xl font-bold transition-all duration-300 ${assessmentState.totalScore >= 0 ? 'text-green-600' : 'text-red-600'}`}>
            ...
            {/* Compact Live Indicator Tracker */}
